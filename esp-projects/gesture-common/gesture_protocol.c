...
void gesture_encode(const gesture_frame_t *f, uint8_t out[GESTURE_FRAME_SIZE]) {
    ...
    put16(out + 8, (uint16_t)f->yaw_cd);
    for (int i = 0; i < 4; ++i) out[10+i] = (uint8_t)(f->uptime_ms >> (8*i));
}
bool gesture_decode(const uint8_t *p, size_t n, gesture_frame_t *f) {
    ...
    int yaw = get16(p+8); f->yaw_cd = (int16_t)(yaw > 32767 ? yaw-65536 : yaw);
    f->uptime_ms = 0;
    for (int i=0; i<4; ++i) f->uptime_ms |= (uint32_t)p[10+i] << (8*i);
    return f->pitch_cd >= -18000 && f->pitch_cd <= 18000 && f->roll_cd >= -18000 && f->roll_cd <= 18000 && f->yaw_cd >= -18000 && f->yaw_cd <= 18000;
}
