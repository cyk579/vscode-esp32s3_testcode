...
| 可选 TFT SCLK / MOSI | PIN_TFT_SCLK / PIN_TFT_MOSI | GPIO13 / GPIO14 |
| 可选 TFT DC / CS / RST | PIN_TFT_DC / PIN_TFT_CS / PIN_TFT_RST | GPIO21 / GPIO0 / GPIO38 |
...
| SCL | PIN_IMU_SCL | GPIO3（释放 E2B；启动/JTAG 复用，必须保证复位时不被外部拉动） |
| VCC / GND | 按模块规格供电，3.3V 逻辑，共地 | 不适用 |
| AD0 | 固定到有效高/低电平，选择 0x69/0x68 | 不适用 |
...
- 默认禁止 0、3、45、46 等启动配置脚。若必须使用，先按实际模组、复位电平和 eFuse 核对，再主动修改 `ALLOW_STRAPPING_PINS`，不要只凭表格编号使用。
...
- 电机电源使用现有驱动板的合适供电，与主控共地。STBY 应在上电/复位时由硬件保持低电平。GPIO1/2 恢复摄像机水平/垂直舵机信号；GPIO17/3 改作 I2C 后不再接 E2A/E2B 编码器信号。
