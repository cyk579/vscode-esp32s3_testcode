...
static void packet(gesture_control_t *c,uint32_t now,uint8_t flags,int pitch,int roll) {
    gesture_frame_t f={.flags=flags,.sequence=(uint16_t)(c->frame.sequence+1),.pitch_cd=pitch,.roll_cd=roll,.uptime_ms=now};
...
static void protocol_test(void) {
    gesture_frame_t f={.flags=3,.sequence=65535,.pitch_cd=-1234,.roll_cd=2500,.uptime_ms=0x12345678};
    uint8_t data[12]; gesture_encode(&f,data);
    const uint8_t expected[]={1,3,255,255,0x2e,0xfb,0xc4,9,0x78,0x56,0x34,0x12};
    assert(!memcmp(data,expected,12));
    gesture_frame_t decoded; assert(gesture_decode(data,12,&decoded));
    assert(decoded.pitch_cd==-1234 && decoded.roll_cd==2500 && decoded.uptime_ms==0x12345678);
    assert(!gesture_decode(data,11,&decoded)); data[0]=2; assert(!gesture_decode(data,12,&decoded));
    data[0]=1; data[1]=128; assert(!gesture_decode(data,12,&decoded));
...
static void states_test(void) {
    ...
    near(c.pwm[0],-30,.01f); near(c.pwm[2],30,.01f); near(c.pwm[1],0,.01f);
...
static void reverse_test(void) {
    ...
    for(uint32_t t=1010;t<2300;t+=10) {
        ...
        if(c.pwm[0]>0) { assert(reached_zero && t-zero>=20); reversed=true; }
    }
    assert(reversed);
}
...
static void mapping_test(void) {
    ...
    float out[3]; gesture_mix(1,1,out); near(out[0],-30,.01f); near(out[1],15,.01f); near(out[2],0,.01f);
    gesture_mix(0,1,out); near(out[0],-30,.01f); near(out[1],30,.01f); near(out[2],-30,.01f);
}
...
int main(void) {
    protocol_test(); states_test(); reverse_test(); wrap_test(); mapping_test(); fusion_test();
