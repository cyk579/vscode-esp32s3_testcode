...
#if CONFIG_SUBJECT3_MUSIC
    err=music_player_start();
    if(err!=ESP_OK) ESP_LOGE("music","Player init: %s",esp_err_to_name(err));
    err=gesture_ble_server_start_extended(link_changed,received,status,music_gatt_register,music_gatt_tick);
#else
