...
| `CURVE_A_SPEED` | 20 | 转弯时 Motor A 的前进分量 |
| `CURVE_D_SPEED` | 20 | 转弯时 Motor D 的前进分量，与 A 保持一致 |
...
| `TURN_DELAY_CYCLES` | 3 | 普通偏线确认后继续前进约 50-60 ms 再转向；强偏差会跳过此延迟 |
...
控制器采用 11:22 版本的转弯流程：每周期进行 5 次多数采样，新方向连续出现两次后确认；普通偏线再经过约 50-60 ms 的车头前探延迟交给 PID，强偏差（`abs(err) >= 20`）直接采用当前稳定读数，避免急弯被延迟队列拖过。偏差方向反转时先输出直行，不回放历史队列中的相反转向；`1001`、`1111` 等零误差状态立即直行。PID 使用中心死区、积分限幅和微分平滑，适合四路数字传感器的离散误差。完全丢线时沿最后偏差方向原地搜索，`1111` 不作为终点停车条件。
