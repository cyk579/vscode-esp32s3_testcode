...
/*
 * 灰度阈值：取 ROI 亮度直方图的暗端和亮端百分位，阈值放在两者之间。
 * 白板配黑胶带时暗端就是胶带、亮端就是板面，中点落在两者中间很宽的空档里，
 * 所以这里不需要精调。对比度不足（画面里没有胶带，或者整片过曝/欠曝）时返回
 * 0，检测器把这一帧当作"没看到线"，不会拿一个瞎猜的阈值去二值化。
 */
/* ROI 必须和 line_detect 真正取样的两条带对齐（远带上沿 38%、近带下沿 99%）。
 * 原来从 15% 开始，把 15%~38% 那段也统计进去了 —— 摄像头俯角低时那是远处的
 * 墙和房间，房间暗会把 light 百分位拉低（胶带不再算黑），背后有窗会把它抬高
 * （白板整片算黑）。同一条赛道换个位置就不行，就是这么来的。 */
#define CAMERA_BINARY_ROI_TOP_PERCENT 35
...
#define CAMERA_BINARY_BLACK_FRACTION_PERCENT 45
/* 下限不要抬高：抬到 75 以上时，光线偏暗的白板会被整片判成黑。 */
#define CAMERA_BINARY_THRESHOLD_MIN 45
#define CAMERA_BINARY_THRESHOLD_MAX 160
#define CAMERA_BINARY_THRESHOLD_SLEW 6
...
typedef struct {
    ...
    uint8_t last_dark;   /* 最近一帧 ROI 的第 2 百分位亮度，诊断用 */
    uint8_t last_light;  /* 最近一帧 ROI 的第 90 百分位亮度 */
                                          ...
                                          uint16_t height)
{
    ...
    /* 存下来给 log 用：只看最终阈值分不清"胶带反光"和"曝光乱跳"，
     * 看 dark/light 这两个原始数就一目了然。 */
    s_display.last_dark = (uint8_t)dark;
    s_display.last_light = (uint8_t)light;
...
static void camera_control_task(void *arg)
{
    ...
    while (true) {
        ...
        if (preview_due) {
            /* 叠加层（条带、线心、状态文字）由 line_follow 的 preview 回调
             * 直接画进这块缓冲区，画完就整帧送屏，不额外开显存。 */
...
{
    if (dark != NULL) {
        *dark = s_display.last_dark;
    }
    if (light != NULL) {
        *light = s_display.last_light;
    }
}
