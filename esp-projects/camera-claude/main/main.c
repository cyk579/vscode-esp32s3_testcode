...
#if CONFIG_EXAMPLE_ENABLE_CAMERA_LINE_FOLLOW
    esp_err_t line_follow_err = line_follow_start();
    if (line_follow_err == ESP_OK) {
        camera_display_set_frame_callback(line_follow_frame_callback, NULL);
        camera_display_set_preview_callback(line_follow_preview_callback, NULL);
