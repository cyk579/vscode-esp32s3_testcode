...
class ProtocolTest {
    @Test fun goldenPacket() {
        val bytes = Protocol.encode(65535, 0x12345678, -12.34f, 25f, true, true, false)
        assertArrayEquals(byteArrayOf(1,3,-1,-1,0x2e,-5,-60,9,0x78,0x56,0x34,0x12),bytes)
    }
    @Test fun invalidSensorNeverEncodesValid() {
        assertEquals(0, Protocol.encode(0,0,Float.NaN,0f,true,false,false)[1].toInt())
    }
