...
class DriveControllerTest {
    @Test fun uiInterruptionRequiresFreshActivationForEverySource() {
        for (source in ControlSource.entries) {
            val controller = ready(source)
            assertTrue(controller.begin(source, 400))
            assertTrue(controller.submit(source, 12f, 8f, 6f, 400, 400))
            controller.stop(410)
            assertNull(controller.output(410))
            assertEquals(0, controller.frame(410)[1].toInt() and 2)
            controller.updateStatus(state(), 800)
            assertFalse(controller.submit(source, 12f, 8f, 6f, 800, 800))
            assertTrue(controller.begin(source, 800))
            assertTrue(controller.submit(source, 12f, 8f, 6f, 800, 800))
        }
    }
