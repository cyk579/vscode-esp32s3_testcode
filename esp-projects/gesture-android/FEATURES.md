...
APP 只有一个 BLE 连接。运动功能选择“摇杆 / 语音 / 手势”，音乐在所有模式下独立工作。UI 组装在 `MainActivity.kt`，运动状态、识别会话、播放命令和传输分别由下表模块管理。
...
只有新启用动作、连接有效、APP 前台、最近 500ms 内车端正常状态、READY 且中立 350ms 后才能起动。语音 800ms 到期；摇杆/手势需要持续的新输入，期限为 200ms。超过期限即撤销，即使随后又有新帧也不能自动恢复本次动作，需重新触摸/启用。

模式切换、释放摇杆、手势丢失、急停、断连和后台均撤销运动。所有来源共享急停与车端 IMU 保护。运动取消不调用 `MediaSink.Stop`。语音点歌开始采音前会停车，但不会停止音乐。
...
| 语音实机 | 有中文服务的手机 + 任意兼容车端 | 权限、取消、800ms 动作、就绪失败不执行；无需手势 |
