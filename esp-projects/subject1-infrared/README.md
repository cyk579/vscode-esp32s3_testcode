...
| ESP32-S3 | HC-SR04 | 用途 |
| --- | --- | --- |
| GPIO40 | TRIG | 10 us 触发脉冲 |
| GPIO39 | ECHO | 回波脉宽测距 |
| 3.3 V | VCC | 本工程按 3.3 V 供电 |
| GND | GND | 必须共地 |

程序在距离小于等于 `5 cm` 时进入避障；距离恢复到 `15 cm` 以上后结束左移。每次显示值由三次测量取中值，无回波时显示 `---` 而不是伪造 `0 cm`。HC-SR04 的 `ECHO` 若实际由 5 V 供电产生 5 V 高电平，必须先分压，即使开发板逻辑使用 3.3 V。
...
当巡线状态下四路红外同时检测到黑线（`ACTIVE=1111`）并持续约 `300 ms`，程序进入 `END` 状态，三个电机停止并保持停止。若赛道中存在宽的十字线，可将 `END_CONFIRM_MS` 调大，避免误停车。
