 ...
 * decoder. The callback receives the decoded RGB565 frame without any
 * display-only conversion. source_threshold is the adaptive grayscale
 * threshold, or zero when the source frame did not have enough contrast. The
 * callback runs in the decoder task and must not block.
 */
