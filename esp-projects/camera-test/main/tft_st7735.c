...
/* Columns are stored top-to-bottom in bits 0..6.  Keeping the small font here
 * avoids pulling a graphics/text library into the low-rate diagnostic path. */
static void font_glyph(char c, uint8_t glyph[5])
{
    ...
    if (c >= '0' && c <= '9') {
        for (size_t i = 0; i < 5; ++i) {
            glyph[i] = digits[(unsigned)(c - '0')][i];
        }
    } else if (c >= 'A' && c <= 'Z') {
        for (size_t i = 0; i < 5; ++i) {
            glyph[i] = upper[(unsigned)(c - 'A')][i];
        }
    } else {
        switch (c) {
        case '-': glyph[1] = 0x08; glyph[2] = 0x08; glyph[3] = 0x08; break;
        case ':': glyph[2] = 0x14; break;
        case '.': glyph[2] = 0x40; break;
        case '=': glyph[1] = 0x14; glyph[2] = 0x14; glyph[3] = 0x14; break;
        case '/': glyph[0] = 0x60; glyph[1] = 0x18; glyph[2] = 0x06; break;
        case '+': glyph[2] = 0x1c; glyph[1] = 0x08; glyph[3] = 0x08; break;
        case '[': glyph[1] = 0x7f; glyph[2] = 0x41; break;
        case ']': glyph[2] = 0x41; glyph[3] = 0x7f; break;
        default: break; /* space and unsupported characters remain blank */
        }
    }
}
...
bool tft_st7735_draw_text(uint16_t x,
                          ...
                          uint16_t background)
{
    if (!s_initialized || text == NULL || x >= TFT_ST7735_WIDTH ||
        y + 7U >= TFT_ST7735_HEIGHT) {
        return false;
    }

    const uint16_t pixel_width = TFT_ST7735_WIDTH - x;
    const size_t max_chars = pixel_width / 6U;
    size_t char_count = 0;
    while (text[char_count] != '\0' && char_count < max_chars) {
        ++char_count;
    }
    if (char_count == 0) {
        char_count = 1; /* still clear one cell when the caller passes "" */
    }

    uint8_t line[TFT_ST7735_WIDTH * 2];
    const uint8_t fg_hi = (uint8_t)(foreground >> 8);
    const uint8_t fg_lo = (uint8_t)foreground;
    const uint8_t bg_hi = (uint8_t)(background >> 8);
    const uint8_t bg_lo = (uint8_t)background;
    const size_t line_bytes = (size_t)pixel_width * 2U;
    for (uint16_t row = 0; row < 7; ++row) {
        for (uint16_t column = 0; column < pixel_width; ++column) {
            line[(size_t)column * 2U] = bg_hi;
            line[(size_t)column * 2U + 1U] = bg_lo;
        }
        for (size_t index = 0; index < char_count; ++index) {
            uint8_t glyph[5];
            font_glyph(text[index], glyph);
            const uint16_t base = (uint16_t)(index * 6U);
            for (uint16_t column = 0; column < 5; ++column) {
                if ((glyph[column] & (1U << row)) == 0) {
                    continue;
                }
                const uint16_t xpixel = base + column;
                if (xpixel >= pixel_width) {
                    break;
                }
                line[(size_t)xpixel * 2U] = fg_hi;
                line[(size_t)xpixel * 2U + 1U] = fg_lo;
            }
        }
        if (set_window(x, (uint16_t)(y + row), TFT_ST7735_WIDTH - 1,
                       (uint16_t)(y + row)) != ESP_OK ||
            write_data(line, line_bytes) != ESP_OK) {
            return false;
        }
    }
    return true;
