...
当前实车默认配置为：巡线开启，TFT 预览、Wi-Fi streaming 与舵机测试关闭。关闭 TFT 时，摄像头解码和巡线控制不经过 SPI 刷屏；需要调试时可显式开启 `Enable TFT status page`，此时 TFT 约每 400 ms 直接显示一帧控制用的解码画面（160x120 或 120x80）并叠加巡线标记：绿点是各扫描行找到的线中心，红十字是最靠近车的种子行，黄点是判定为宽黑段（折角横条/终点横杆）的行。屏幕上不显示数值，状态与阈值请看 UART0 串口日志。
...
如果屏幕板另有 `BLK`/`LED` 背光引脚，将它接到 `3V3`。摄像头仍必须接 **5 V**，不能把摄像头 VBUS 和屏幕 VCC 混接。TFT 预览是可选调试功能，默认关闭以保持控制帧率。
