...
当前实车默认配置为：巡线和 TFT 预览开启，Wi-Fi streaming 与舵机测试关闭。TFT 约每 400 ms 直接显示一帧控制用的解码画面（160x120 或 120x80）并叠加巡线标记：绿点是各扫描行找到的线中心，红十字是最靠近车的种子行，黄点是判定为宽黑段（折角横条/终点横杆）的行。屏幕上不显示数值，状态与阈值请看 UART0 串口日志。
